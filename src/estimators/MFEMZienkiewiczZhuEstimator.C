...
registerMooseObject("PlatypusApp", MFEMZienkiewiczZhuEstimator);
...
mfem::ErrorEstimator * MFEMZienkiewiczZhuEstimator::createEstimator()
{
  ...
  int sdim = problem._pmesh->SpaceDimension();
  ...
  */
  ...
  _flux_fes = std::make_unique<mfem::ParFiniteElementSpace>(problem._pmesh.get(), _flux_fec.get(), sdim);
  
  _smooth_flux_fec = std::make_unique<mfem::H1_FECollection>(order, dim);
  _smooth_flux_fes = std::make_unique<mfem::ParFiniteElementSpace>(problem._pmesh.get(), _smooth_flux_fec.get(), dim);
  
  // fetch the grid function we need
  auto gridfunction = problem._gridfunctions.GetShared( _test_var_name );
