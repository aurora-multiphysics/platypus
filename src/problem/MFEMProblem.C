...
{
  FEProblemBase::addUserObject(user_object_name, name, parameters);
  MFEMCoefficient & coef(getUserObject<MFEMCoefficient>(name));
  getProperties().declareScalar(name, coef.getCoefficient());
}
...
{
  FEProblemBase::addUserObject(user_object_name, name, parameters);
  MFEMVectorCoefficient & vec_coef(getUserObject<MFEMVectorCoefficient>(name));
  getProperties().declareVector(name, vec_coef.getVectorCoefficient());
}
