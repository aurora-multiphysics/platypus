...
# This file has been modified from its original form to fix missing code
# coverage on closing braces in files with the .C extension.
...
then
  ...
  if echo "$network" | grep -m1 '\(.c\.cpp\|.C\|.cxx\|.h\|.hpp\|.m\|.swift\|.vala\)$' 1>/dev/null;
  then
    # skip brackets
